    ...
    public static class TensorExtensions
    {
            ...
            float minProbability, float overlapThreshold)
        {
            ...
            if (result.Count == 0) { 
                ...
                if (mainCamera != null) {
                    var centerPosition = mainCamera.transform.position + mainCamera.transform.forward *2f;

                    var dummyItem = new YoloItem(
                        "Dummy Object", new Vector2(centerPosition.x - 0.1f, centerPosition.y - 0.1f), 
                        new Vector2(centerPosition.x + 0.1f, centerPosition.y + 0.1f), 1.0f
                    );
